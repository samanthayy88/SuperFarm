...
| **Schedule** | Daily work plan filterable by worker, farm, plot and date range, in calendar / by-worker / list views. Several tasks and several plots can be entered at once — task rows appear as you type — and each combination is scheduled across the repeat period. Each task carries free-text **remarks** for the worker. Created tasks can be edited (including adding more plots, which splits the task) or deleted, and their status cycled by clicking the badge |
